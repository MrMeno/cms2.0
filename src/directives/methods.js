...
export function checks(datas) {
    ...
    var data = datas;
    var oj = new Object();
    var reg = /^1[3|4|5|7|8][0-9]{9}$/;//手机号码校验
...
export function roleSerialize(data){//返回链接是否显示，页面url，以及页面内的接口是否能用
    var arr=new Array();
    for(var i=0;i<data.length;i++){
        if(data[i].url.trim()=='/api/product/save'){
            arr.push({showUrl:true,url:'/userCenter/pubMedia',apiA:data[i].action})
        }
       else if(data[i].url.trim()=='/api/product/update'){
            arr.push({showUrl:true,url:'/userCenter/edMedia',apiA:data[i].action})
        }
